...
public abstract class AbstractRulesEvaluator implements RulesEvaluator {
    ...
    CompletableFuture<List<Match>> scheduledAdvanceTimeToMills(long millis) {
        ...
        if (log.isInfoEnabled()) {
            log.info("Automatic advance of " + (millis - currentTime) + " milliseconds" );
        }

        // Matches generated by a scheduled clock advance are always written on the async channel
        return asyncExecutor.submit(() -> writeResponseOnChannel(internalAdvanceTime(millis - currentTime, TimeUnit.MILLISECONDS) ) );
    }

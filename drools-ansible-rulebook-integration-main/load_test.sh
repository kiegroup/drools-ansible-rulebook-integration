...
# Usage: ./load_test.sh
# This script runs a series of test files. While you can confirm STDOUT, the results are redirected to result.txt through STDERR.
...
# Truncate (or create) result.txt so we start fresh each time
> result.txt
...
echo "All runs complete. See result.txt for any STDERR output (testfile, usedMemory, timeTaken)."
